...
export class SlackAPI {
  constructor() {
    ...
    if (!this.webhookUrl) {
      console.warn(
        "Warning: SLACK_WEBHOOK_URL not set. Slack notifications will be disabled.",
      );
    }
  }
  ...
  async sendAlert(message) {
    if (!this.webhookUrl) {
      await this.logging.logWarning(
        "slack",
        "Slack notification skipped - webhook URL not configured",
      );
      return;
    }
