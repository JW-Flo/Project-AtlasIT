  ...
  const tests = [
    {
      ...
      validate: async (r) => {
        ...
        if (missing.length)
          throw new Error("health fields invalid: " + missing.join(","));
      },
    },
    {
      ...
      validate: async (r) => {
        const j = await r.json();
        if (!Array.isArray(j.frameworkSummary))
    ...
    },
    {
      ...
      validate: async (r) => {
        ...
        if (!t.toLowerCase().includes("atlasit console"))
    ...
    },
    {
      ...
      validate: async (r) => {
        if (r.status !== 307 && r.status !== 308) {
          const t = await r.text();
          if (!t.toLowerCase().includes("atlasit console"))
            throw new Error("root redirect/content fail");
        }
      },
    },
  ];
