...
async function run() {
  ...
  const tests = [
    ...
    },
    {
      ...
      validate: async (res) => {
        const data = await res.json();
        if (!Array.isArray(data.frameworkSummary)) throw new Error('frameworkSummary missing');
      }
    },
    {
      path: '/console',
      validate: async (res) => {
        ...
        if (!text.toLowerCase().includes('atlasit console')) throw new Error('console page missing text');
      }
    },
    {
      path: '/',
      validate: async (res) => {
        if (res.status !== 307 && res.status !== 308) {
          const body = await res.text();
            if (!body.toLowerCase().includes('atlasit console')) throw new Error('root not redirecting & content mismatch');
        }
      }
    }
  ];
  ...
  for (const t of tests) {
    ...
    try {
      const response = await worker.fetch(makeRequest(t.path), { waitUntil: () => {} });
