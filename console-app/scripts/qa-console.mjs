  ...
  results.push(
    await check("/api/health", (b) => {
      ...
      for (const g of groups)
        if (!b.resources[g]) throw new Error(`missing resource group ${g}`);
    })
  );
  results.push(
    await check("/api/mock/compliance/snapshot", (b) => {
      if (!b.frameworkSummary || !Array.isArray(b.frameworkSummary))
        throw new Error("missing frameworkSummary");
    })
  );
  results.push(
    await check("/console", (t) => {
      if (typeof t !== "string" || !t.toLowerCase().includes("atlasit console"))
        throw new Error("console page content mismatch");
    })
  );
  results.push(
    await check("/", (t) => {
