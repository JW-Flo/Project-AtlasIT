...
function generateBadgeSvg(tenantName: string, score: number, grade: string): string {
...
export const GET: RequestHandler = async ({ params, url, platform }) => {
  ...
  // Get overall score (average across frameworks)
  ...
  const avgScore =
    scores.length > 0 ? Math.round(scores.reduce((s, r) => s + r.score, 0) / scores.length) : 0;
