...
export const GET: RequestHandler = async ({ params, locals }) => {
  ...
  const roleRow = await queryPgOne(`SELECT * FROM roles WHERE id = $1 AND tenant_id = $2`, [
    params.id!,
    tenantId,
  ]);
...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
  ...
  const existing = await queryPgOne(`SELECT id FROM roles WHERE id = $1 AND tenant_id = $2`, [
    params.id!,
    tenantId,
  ]);
  ...
  const updated = await queryPgOne(`SELECT * FROM roles WHERE id = $1 AND tenant_id = $2`, [
    params.id!,
    tenantId,
  ]);
