...
export const GET: RequestHandler = async ({ url, locals, platform }) => {
  ...
  switch (widget) {
    case "compliance-scores":
      return exportComplianceScores(user, framework, dateStr);
    case "compliance-trend":
      return exportComplianceTrend(user, days, dateStr);
    case "evidence-feed":
      return exportEvidenceFeed(user, since, framework, dateStr);
    case "evidence-volume":
      return exportEvidenceVolume(user, days, dateStr);
    case "adapter-health":
      return exportAdapterHealth(user, dateStr);
    case "automation-metrics":
      return exportAutomationMetrics(user, days, dateStr);
    case "automation-recent":
      return exportAutomationRecent(user, dateStr);
    case "jml-metrics":
      return exportJmlData(user, since, dateStr, "metrics");
    case "jml-adapter-provisions":
      return exportJmlData(user, since, dateStr, "provisions");
    case "workflow-runs":
      return exportWorkflowRuns(user, dateStr);
    case "security-posture":
      return exportSecurityPosture(user, dateStr);
    default:
      return json({ error: `Unknown widget: ${widget}` }, { status: 400 });
  }
};

async function exportComplianceScores(user: any, framework: string | null, dateStr: string) {
  let query = "SELECT * FROM compliance_scores WHERE tenant_id = $1";
  const params: unknown[] = [user.tenantId];
  if (framework) {
    query += " AND framework = $2";
    params.push(framework);
  }
  const results = await queryPg(query, params);
...
async function exportComplianceTrend(user: any, days: number, dateStr: string) {
  const since = new Date(Date.now() - days * 86400000).toISOString();
  const results = await queryPg(
    "SELECT framework, score, recorded_at FROM compliance_history WHERE tenant_id = $1 AND recorded_at >= $2 ORDER BY recorded_at",
    [user.tenantId, since],
  );
  const rows = toCamel(results) as any[];
  const csv = buildCSV(
...
async function exportEvidenceFeed(
  ...
  let query = "SELECT * FROM compliance_evidence WHERE tenant_id = $1 AND created_at >= $2";
  const params: unknown[] = [user.tenantId, since];
  if (framework) {
    query += " AND control_id ILIKE $3";
...
async function exportEvidenceVolume(user: any, days: number, dateStr: string) {
  const since = new Date(Date.now() - days * 86400000).toISOString();
  const results = await queryPg(
    `SELECT TO_CHAR(created_at, 'IYYY-"W"IW') as week, COUNT(*) as count
     FROM compliance_evidence WHERE tenant_id = $1 AND created_at >= $2
...
async function exportAdapterHealth(user: any, dateStr: string) {
  const results = await queryPg(
    `SELECT adapter_slug, collected_at, items_count, error
     FROM adapter_collection_health WHERE tenant_id = $1
...
async function exportAutomationMetrics(user: any, days: number, dateStr: string) {
  const since = new Date(Date.now() - days * 86400000).toISOString();
  const results = await queryPg(
...
async function exportAutomationRecent(user: any, dateStr: string) {
  const results = await queryPg(
...
async function exportJmlData(
  ...
  const results = await queryPg(
    "SELECT * FROM workflow_runs WHERE tenant_id = $1 AND started_at >= $2 ORDER BY started_at DESC",
    [user.tenantId, since],
  );
  const rows = toCamel(results) as any[];
...
async function exportWorkflowRuns(user: any, dateStr: string) {
  const results = await queryPg(
    "SELECT * FROM workflow_runs WHERE tenant_id = $1 ORDER BY started_at DESC LIMIT 100",
    [user.tenantId],
  );
  const rows = toCamel(results) as any[];
  const csv = buildCSV(
...
async function exportSecurityPosture(user: any, dateStr: string) {
  // Aggregate from incidents + access reviews
  const [incidents, reviews] = await Promise.all([
    queryPg("SELECT status, COUNT(*) as cnt FROM incidents WHERE tenant_id = $1 GROUP BY status", [
      user.tenantId,
    ]).catch(() => []),
    queryPg(
      "SELECT status, COUNT(*) as cnt FROM access_review_campaigns WHERE tenant_id = $1 GROUP BY status",
      [user.tenantId],
    ).catch(() => []),
  ]);
