...
/** Generate a smooth synthetic trend from a current score over N weeks */
function syntheticTrend(
  currentScore: number,
  weeks: number,
): Array<{ week: string; score: number }> {
  const result: Array<{ week: string; score: number }> = [];
...
export const GET: RequestHandler = async ({ locals, platform }) => {
  ...
  // ── 1. Framework scores from compliance_scores ──────────────────────────────
  let frameworkBreakdown: Array<{
    framework: string;
    score: number;
    grade: string;
  ...
  try {
    ...
    // compliance_scores table may not exist yet
  }
  ...
  try {
    ...
    // compliance_history may not exist
  }

  // Synthetic fallback when < 3 real points
  ...
  // ── 7. Security posture (incidents + access reviews) ─────────────────────────
  let securityPosture = {
    openIncidents: 0,
    resolvedIncidents: 0,
    criticalIncidents: 0,
  ...
  try {
    ...
    securityPosture.accessReviewCompletionRate =
      arTotal > 0 ? Math.round((arCompleted / arTotal) * 100) : 0;
  } catch {
    // access_reviews table may not exist
  }

  // ── 8. Top risks: controls with lowest scores ────────────────────────────────
  interface TopRiskControl {
    controlRef: string;
    title: string;
    framework: string;
    score: number;
  ...
  // If we have no controls from preferences, try compliance_scores derived risk
  if (topRisks.length === 0 && frameworkBreakdown.length > 0) {
    topRisks = frameworkBreakdown
      .sort((a, b) => a.score - b.score)
      .slice(0, 5)
      .map((f) => ({
