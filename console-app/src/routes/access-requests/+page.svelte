  ...
  {:else}
    <table class="w-full text-xs border-collapse">
      <thead class="text-neutral-400">
        <tr
          ><th class="p-2 text-left">ID</th><th class="p-2 text-left"
            >Subject</th
          ><th class="p-2 text-left">Resource</th><th class="p-2 text-left"
            >Status</th
      ...
      <tbody>
        {#each items as r}
          <tr class="border-t border-neutral-800">
            <td class="p-2">{r.id}</td><td class="p-2">{r.subject}</td><td
              class="p-2">{r.resource}</td
            ...
            <td class="p-2 flex gap-2 flex-wrap">
              {#if r.status === "pending"}
                ...
                >
                <button
                  class="px-2 py-0.5 bg-red-600 rounded"
                  on:click={() => act(r.id, "deny")}>Deny</button
                >
              {:else if r.status === "approved"}
                <button
                  class="px-2 py-0.5 bg-indigo-600 rounded"
