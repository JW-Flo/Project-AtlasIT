...
import { queryPg, queryPgOne, getPgPool } from "$lib/server/pg";
...
export async function expireOverdueCampaigns(db: D1Database, tenantId: string): Promise<number> {
  const now = new Date().toISOString();
  const result = await db
    .prepare(
      `UPDATE access_review_campaigns
...
export async function listCampaignsPg(tenantId: string): Promise<AccessReviewCampaign[]> {
...
export async function createCampaignPg(
  tenantId: string,
  input: CreateCampaignInput,
): Promise<AccessReviewCampaign> {
  const id = crypto.randomUUID();
  ...
  // Populate NHI items if scope is nhi-based
  let totalItems = 0;
  if (scope.startsWith("nhi")) {
...
async function populateNhiItemsPg(
  tenantId: string,
  campaignId: string,
  scope: string,
): Promise<AccessReviewItem[]> {
