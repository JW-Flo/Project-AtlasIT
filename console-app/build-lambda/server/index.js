...
import { t as text_decoder, b as base64_decode, r as root, d as decode_pathname, n as normalize_path, a as disable_search, c as decode_params, v as validate_layout_server_exports, e as validate_layout_exports, f as validate_page_server_exports, g as validate_page_exports, h as text_encoder, i as resolve, m as make_trackable, j as base64_encode } from './chunks/root-okDAyOJ_.js';
import { D as DevalueError, i as is_primitive, g as get_type, a as is_plain_object, e as enumerable_symbols, s as stringify_key, b as stringify_string, v as valid_array_indices, c as DEV, u as uneval } from './chunks/renderer-CwxN8JkH.js';
import { r as readable, w as writable } from './chunks/index-C1X1AO8K.js';
...
<html lang="en" class="h-full" data-theme="dark">\r
  <head>\r
    ...
    <title>AtlasIT Console</title>\r
    <link rel="preconnect" href="https://rsms.me/" />\r
    <link rel="stylesheet" href="https://rsms.me/inter/inter.css" />\r
    <script>\r
      // Blocking theme init — prevents white flash (FOUC) on page load.\r
      // Reads localStorage before first paint and sets data-theme + .dark class\r
      // so both CSS custom properties and Tailwind dark: utilities work immediately.\r
      (function() {\r
        var t = 'dark';\r
        try { var s = localStorage.getItem('atlasit.theme'); if (s === 'light' || s === 'dark') t = s; } catch(e) {}\r
        document.documentElement.setAttribute('data-theme', t);\r
        if (t === 'dark') document.documentElement.classList.add('dark');\r
        else document.documentElement.classList.remove('dark');\r
      })();\r
\r
      // Static SPA mode: intercept SvelteKit __data.json fetches (no server data in static bundle)\r
      // Must run before SvelteKit bundle bootstraps.\r
      (function() {\r
        var _fetch = window.fetch.bind(window);\r
        window.fetch = function(input, init) {\r
          var url = typeof input === 'string' ? input : (input instanceof URL ? input.href : (input && input.url) || '');\r
          if (url.indexOf('__data.json') !== -1) {\r
            return Promise.resolve(new Response(\r
              JSON.stringify({ type: 'data', nodes: [null, null, null] }),\r
              { status: 200, headers: { 'content-type': 'application/json' } }\r
            ));\r
          }\r
          return _fetch(input, init);\r
        };\r
      })();\r
    <\/script>\r
...
async function get_hooks() {
  ...
  let init;
  ({ handle, handleFetch, handleError, handleValidationError, init } = await import('./chunks/hooks.server-CjiUttn5.js'));
  let reroute;
  let transport;
  return {
    handle,
