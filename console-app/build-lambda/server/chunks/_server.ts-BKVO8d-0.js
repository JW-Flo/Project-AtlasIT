...
const POST = async ({ params, request, platform, locals }) => {
  ...
  try {
    ...
    if (!body.reviewerEmails.every((e) => typeof e === "string" && e.trim())) {
      return json(
  ...
  try {
    ...
    for (const email of body.reviewerEmails) {
      ...
      ).bind(approvalId, id, id, tenantId, email.trim(), now).run();
    }
    try {
      await writeAudit(db, {
        tenantId,
    ...
    try {
      ...
      for (const email of body.reviewerEmails) {
        const reviewer = await db.prepare("SELECT id FROM console_users WHERE email = ? AND tenant_id = ?").bind(email.trim(), tenantId).first();
...
export { POST };
//# sourceMappingURL=_server.ts-BKVO8d-0.js.map
