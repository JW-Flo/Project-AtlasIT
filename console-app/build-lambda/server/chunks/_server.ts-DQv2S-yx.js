...
const POST = async ({ request, locals, platform }) => {
  ...
  const billing = await db.prepare("SELECT plan FROM tenant_billing WHERE tenant_id = ?").bind(user.tenantId).first();
...
export { GET, POST };
//# sourceMappingURL=_server.ts-DQv2S-yx.js.map
