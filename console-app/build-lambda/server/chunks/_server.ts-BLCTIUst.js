...
const GET = async ({ params, platform, locals }) => {
  ...
  try {
    ...
    return json({
      ...
      approvals: (approvalsResult.results ?? []).map((a) => ({
        id: a.id,
...
const PUT = async ({ params, request, platform, locals }) => {
  ...
  try {
    const policy = await db.prepare(`SELECT id, version, status FROM policies WHERE id = ? AND tenant_id = ?`).bind(id, tenantId).first();
    if (!policy) return json({ error: "Policy not found" }, { status: 404 });
    ...
    ).bind(body.content, newVersion, now, id, tenantId).run();
    try {
      await writeAudit(db, {
        tenantId,
...
export { GET, PUT };
//# sourceMappingURL=_server.ts-BLCTIUst.js.map
