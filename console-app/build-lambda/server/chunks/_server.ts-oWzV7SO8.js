...
const GET = async ({ params, locals, platform }) => {
  ...
  return json({ members: toCamel(members) });
};
const POST = async ({
  params,
  request,
  ...
  ).bind(groupId, tenantId).first();
  if (!group) return json({ error: "group not found" }, { status: 404 });
  const dirUser = await db.prepare(
  ...
  ).bind(groupId, userId, tenantId).first();
  ...
  await db.prepare(
    `INSERT INTO directory_memberships (id, tenant_id, group_id, user_id, created_at)
       VALUES (?, ?, ?, ?, ?)`
  ).bind(newId, tenantId, groupId, userId, now).run();
  await writeAudit(db, {
    tenantId,
...
const DELETE = async ({
  ...
  ).bind(groupId, userId, tenantId).run();
  await writeAudit(db, {
    tenantId,
    actorUserId: user.userId ?? user.id,
    actorEmail: user.email,
...
//# sourceMappingURL=_server.ts-oWzV7SO8.js.map
