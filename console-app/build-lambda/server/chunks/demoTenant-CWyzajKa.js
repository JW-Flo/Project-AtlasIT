...
function resolveDemoTenantConfig(env) {
  const emailRaw = String(env?.DEMO_SEEDED_EMAIL ?? DEFAULT_DEMO_EMAIL).trim().toLowerCase();
  const passwordRaw = String(env?.DEMO_SEEDED_PASSWORD ?? "").trim();
  const tenantIdRaw = String(env?.DEMO_TENANT_ID ?? DEFAULT_DEMO_TENANT_ID).trim().toLowerCase();
...
async function resetDemoTenant(db, cfg) {
  ...
  ).bind(cfg.tenantId, cfg.tenantName, tenantSlug, now, now).run();
  await db.prepare("DELETE FROM console_users WHERE tenant_id = ? OR id = ?").bind(cfg.tenantId, DEMO_USER_ID).run();
  await db.prepare(
  ...
  ).bind(demoAccessCampaignId, cfg.tenantId, DEMO_USER_ID, now).run();
  const prefs = [
  ...
  for (const [key, value] of prefs) {
    ...
    ).bind(cfg.tenantId, key, JSON.stringify(value), now).run();
  }
}

export { resetDemoTenant as a, resolveDemoTenantConfig as r };
//# sourceMappingURL=demoTenant-CWyzajKa.js.map
