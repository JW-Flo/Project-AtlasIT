...
const GET = async ({ locals, platform, url }) => {
  ...
  try {
    ...
    if (row?.value) {
      ...
      controls = all.filter(
        (c) => (c.framework || "").toUpperCase().replace(/[^A-Z0-9]/g, "") === framework.toUpperCase().replace(/[^A-Z0-9]/g, "")
      ).map((c) => ({
        id: c.id,
        name: c.name || c.id,
  ...
  let policyCount = 0;
  try {
    const row = await db.prepare("SELECT COUNT(*) as cnt FROM policies WHERE tenant_id = ? AND status = 'approved'").bind(tenantId).first();
    policyCount = row?.cnt ?? 0;
  } catch {
  }
  let lastAccessReview = null;
  try {
    const row = await db.prepare(
  ...
  if (notStarted.length > 0) {
    checklist.push({
      ...
      description: `Controls with no activity: ${notStarted.slice(0, 5).map((c) => c.id).join(", ")}${notStarted.length > 5 ? "..." : ""}`,
      status: "incomplete",
  ...
  if (inProgress.length > 0) {
    checklist.push({
      ...
      description: "These controls have some evidence but aren't fully implemented. Add more evidence or attestations.",
      status: "warning",
  ...
  if (controlsWithoutEvidence.length > 0) {
    checklist.push({
      ...
      description: `Missing evidence for: ${controlsWithoutEvidence.slice(0, 5).map((c) => c.id).join(", ")}${controlsWithoutEvidence.length > 5 ? "..." : ""}`,
      status: "incomplete",
  ...
  if (ctx.evidenceStats.staleCount > 0) {
    checklist.push({
      ...
      description: "Auditors expect recent evidence. Trigger evidence re-collection for these controls.",
      status: "warning",
      href: "/console/compliance/feed"
    });
  }
  ...
  if (policyCount === 0) {
    checklist.push({
      ...
      description: "Every compliance framework requires documented policies. Generate and approve policies before audit.",
      status: "incomplete",
    ...
    checklist.push({
      ...
      description: policyCount >= requiredMin ? "Policy coverage looks sufficient." : `Consider generating more policies. ${framework} typically requires at least ${requiredMin}.`,
  ...
  if (!lastAccessReview) {
    checklist.push({
      ...
      description: "Periodic access reviews are required by most frameworks. Start your first review campaign.",
      status: "incomplete",
      href: "/console/access-reviews"
    });
  } else {
    const daysSince = Math.floor(
      (Date.now() - new Date(lastAccessReview).getTime()) / (1e3 * 60 * 60 * 24)
    );
    checklist.push({
      id: `audit-${++id}`,
      category: "access_review",
      title: `Last access review completed ${daysSince} day${daysSince !== 1 ? "s" : ""} ago`,
      description: daysSince > 90 ? "Access reviews should be conducted quarterly. Start a new review." : "Access review is recent and audit-ready.",
  ...
  ctx.adapterHealth.filter((a) => !a.error);
  ...
  const readinessScore = checklist.length > 0 ? Math.round(completeItems / checklist.length * 100) : 0;
  const response = {
    framework,
    readinessScore,
...
export { GET };
//# sourceMappingURL=_server.ts-_8eu6i18.js.map
