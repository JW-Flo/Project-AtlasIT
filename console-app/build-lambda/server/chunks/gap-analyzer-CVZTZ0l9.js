...
z.object({
  ...
  endpointLimits: z.record(z.string().min(1), z.number().int().positive()).default({}),
  namespaceBinding: z.string().min(1).default("RATE_LIMIT_KV")
});
z.object({
  contentSecurityPolicy: z.string().min(1).default("default-src 'self'; frame-ancestors 'none'; object-src 'none'; base-uri 'self';"),
  strictTransportSecurity: z.string().min(1).default("max-age=63072000; includeSubDomains; preload"),
...
new TextEncoder().encode("atlasit-credential-vault-v1");
new TextEncoder().encode("credential-encryption");
const ACTION_COMPLIANCE_MAP = {
...
const CONTROL_SPECIFIC_RECOMMENDATIONS = {
  // Audit log controls — identified as weak in compliance review
  "CC7.2": "Enable anomalous activity monitoring. Connect SIEM or log management adapter and configure anomaly detection rules",
  "CC7.5": "Establish audit log retention policy. Configure log storage with retention periods and tamper-proof archival in connected apps",
  "A.12.4.1": "Enable comprehensive event logging across all connected apps. Verify each adapter collects audit log evidence",
  "A.12.4.2": "Protect log information from tampering. Enable immutable log storage and configure access controls on log management systems",
  "A.12.4.3": "Collect administrator and operator activity logs from all connected apps with privileged access tracking",
  "A.12.4.4": "Enable NTP/clock synchronisation evidence from connected infrastructure. Verify timestamps are consistent across adapters",
  "A.12.6.1": "Connect vulnerability scanning tools (e.g., Snyk, Qualys) to collect technical vulnerability management evidence",
  // Access control depth — identified as needing more than "groups configured"
  "CC5.2": "Implement segregation of duties checks. Configure rules that flag when users hold conflicting roles across apps",
  "CC6.5": "Set up access monitoring rules. Create automation that detects and alerts on suspicious access patterns",
  "A.9.4.2": "Verify MFA enforcement evidence from identity providers. Connect Okta/M365/Google Workspace MFA policy data",
  "PR.AC-5": "Configure SoD (Separation of Duties) rules to detect conflicting access grants across connected apps",
  // Offboarding — identified as under-evidenced
  "CC6.7": "Create offboarding automation rules that revoke access across ALL connected apps, emit evidence, and create audit trail",
  "A.7.3.1": "Configure JML leaver workflows with session revocation, device wipe triggers, and shared credential rotation steps",
  // HIPAA-specific — need end-to-end audit evidence
  "164.312(b)": "Audit controls require evidence of log generation, retention, review, and anomaly detection across all PHI-relevant systems. Connect healthcare app adapters",
  "164.308(a)(1)(ii)(D)": "Configure regular information system activity review. Create scheduled automation rules to collect and review access logs",
  "164.308(a)(5)(ii)(C)": "Enable log-in monitoring evidence. Connect identity provider adapters and configure failed login alerting rules",
  "164.312(a)(2)(ii)": "Document emergency access (break-glass) procedures and create automation rules to track emergency access usage",
  "164.312(c)(1)": "Enable data integrity controls. Verify content-addressed evidence storage and hash verification are active",
  "164.312(e)(1)": "Verify TLS/encryption evidence from connected apps. Connect infrastructure adapters that report encryption status",
  // GDPR process controls
  "Art.5(2)": "Establish accountability records. Generate data processing activity logs and controller/processor documentation",
  "Art.25": "Document data protection by design measures. Generate policies covering privacy-first architecture decisions",
  "Art.32": "Verify security of processing controls. Collect encryption, access control, and pseudonymization evidence from connected apps",
  "Art.33": "Configure breach notification workflows. Create automation rules that detect potential breaches and trigger notification procedures",
  // NIST CSF detection and response
  "DE.AE-3": "Set up event correlation rules. Connect SIEM adapters and configure cross-app anomaly detection automation",
  "DE.CM-3": "Enable personnel activity monitoring. Create rules that track privileged user actions across connected apps",
  "RS.AN-1": "Configure investigation workflows. Create automation rules for incident triage with evidence collection from affected apps"
};
function buildRecommendation(controlId, controlName, gapType, staleDays) {
...
function assignPriority(gapType, controlId) {
  if (gapType === "failing")
    return "critical";
...
async function analyzeComplianceGaps(db, tenantId, frameworks) {
  ...
  const { results: evidenceRows } = await db.prepare(`SELECT framework, control_id,
       ...
       WHERE tenant_id = ?
       GROUP BY framework, control_id`).bind(tenantId).all();
  ...
  for (const framework of frameworks) {
    ...
    for (const control of controls) {
      ...
      if (!evidence) {
        missingCount++;
        const { recommendation, suggestedAction } = buildRecommendation(control.controlId, control.controlName, "missing", null);
        gaps.push({
      ...
      if (evidence.latest_status === "fail") {
        failingCount++;
        const daysSince2 = Math.floor((now - new Date(evidence.latest_at).getTime()) / (24 * 60 * 60 * 1e3));
        const { recommendation, suggestedAction } = buildRecommendation(control.controlId, control.controlName, "failing", daysSince2);
        gaps.push({
          controlId: control.controlId,
          controlName: control.controlName,
          framework,
      ...
      const daysSince = Math.floor((now - new Date(evidence.latest_at).getTime()) / (24 * 60 * 60 * 1e3));
      if (daysSince > STALE_THRESHOLD_DAYS) {
        staleCount++;
        const { recommendation, suggestedAction } = buildRecommendation(control.controlId, control.controlName, "stale", daysSince);
        gaps.push({
          controlId: control.controlId,
          controlName: control.controlName,
          framework,
...
export { ACTION_COMPLIANCE_MAP as A, Logger as L, analyzeComplianceGaps as a, lookupAuditEvidence as l };
//# sourceMappingURL=gap-analyzer-CVZTZ0l9.js.map
