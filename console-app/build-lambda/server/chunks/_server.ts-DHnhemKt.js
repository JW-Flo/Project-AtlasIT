const GET = async ({ url, platform, locals }) => {
  ...
  return json({
    activities: (results ?? []).map((row) => {
      ...
      const payload = te?.payload ?? te;
      return {
        id: row.id,
        eventType: te?.type ?? "automation",
        title: row.rule_name ?? "Automation execution",
        detail: payload?.email ? `${row.status} — ${payload.email}` : `${row.status} — ${row.actions_run ?? 0} action(s)`,
...
export { GET };
//# sourceMappingURL=_server.ts-DHnhemKt.js.map
