...
const GET = async ({ params, platform, locals }) => {
  ...
  const { id } = params;
  try {
    const incident = await db.prepare("SELECT * FROM incidents WHERE id = ? AND tenant_id = ?").bind(id, tenantId).first();
    if (!incident) {
      return json({ error: "Incident not found" }, { status: 404 });
    }
    const { results: timelineRows } = await db.prepare(
...
export { GET };
//# sourceMappingURL=_server.ts-Dox9jwmo.js.map
