...
const GET = async ({ params, locals, platform }) => {
  ...
  try {
    ...
    const auditMapped = auditRows.map((entry) => {
      const e = { ...entry };
      e.details = parseJsonField(e.details);
      return e;
    });
    const credential = mapNhiRow(row);
    return json({
...
const PATCH = async ({ params, request, locals, platform }) => {
  ...
  if (!db) return json({ error: "database unavailable" }, { status: 503 });
  const { id } = params;
  const existing = await db.prepare(`SELECT id, owner_email, status FROM nhi_credentials WHERE id = ? AND tenant_id = ?`).bind(id, tenantId).first();
  if (!existing) return json({ error: "not found" }, { status: 404 });
  const body = await request.json().catch(() => null);
  if (!body) return json({ error: "invalid request body" }, { status: 400 });
  const { ownerEmail, status } = body;
  const ALLOWED_STATUSES = ["active", "revoked", "rotation_pending"];
  if (status !== void 0 && !ALLOWED_STATUSES.includes(status)) {
  ...
  await db.prepare(`UPDATE nhi_credentials SET ${updates.join(", ")} WHERE id = ? AND tenant_id = ?`).bind(...binds).run();
  const changes = {};
  if (ownerEmail !== void 0) changes.owner_email = ownerEmail;
  if (status !== void 0) changes.status = status;
  await db.prepare(
  ...
  return json({ credential: toCamel(mapNhiRow(updated)) });
};
const DELETE = async ({ params, locals, platform }) => {
  ...
  if (!db) return json({ error: "database unavailable" }, { status: 503 });
  const { id } = params;
  const existing = await db.prepare(`SELECT id, status FROM nhi_credentials WHERE id = ? AND tenant_id = ?`).bind(id, tenantId).first();
  if (!existing) return json({ error: "not found" }, { status: 404 });
  const now = (/* @__PURE__ */ new Date()).toISOString();
  await db.prepare(
...
//# sourceMappingURL=_server.ts-C7sbU_t9.js.map
