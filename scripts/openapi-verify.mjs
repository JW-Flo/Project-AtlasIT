...
const REQUIRED_SCHEMAS = ['ComplianceSnapshot', 'FrameworkSummary', 'Risk', 'Policy', 'Error'];
...
async function main() {
  try {
    ...
    if (!spec.openapi || !/^3\.1\./.test(spec.openapi)) {
      fail(`OpenAPI version must be 3.1.x (found: ${spec.openapi || 'missing'})`);
    }
    if (!spec.paths) fail('Missing paths section');
    if (!spec.components || !spec.components.schemas) fail('Missing components.schemas section');
