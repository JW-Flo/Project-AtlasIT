...
const REQUIRED_SERVICE_KEYS = ["ONBOARDING_API_KEY", "ORCHESTRATOR_API_KEY"];
...
function has(v) {
  return process.env[v] && process.env[v].trim() !== "";
}
...
if (process.env.FEATURE_IDP_OKTA === "true") {
  for (const k of OPTIONAL_IDP)
    if (!has(k)) missing.push(k + " (required because FEATURE_IDP_OKTA=true)");
}
