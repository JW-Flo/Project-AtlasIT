...
const MAX_TOTAL_BYTES =
  parseInt(process.env.MAX_TOTAL_BYTES || "", 10) || 2 * 1024 * 1024; // 2MB
...
try {
  ...
  const result = {
    ...
    largest,
    count: assets.length,
    maxAssetBudget: MAX_BYTES,
    maxTotalBudget: MAX_TOTAL_BYTES,
    assets,
  };
  console.log(JSON.stringify(result, null, 2));
  let fail = false;
  if (largest && largest.bytes > MAX_BYTES) {
    console.error(
      `Asset ${largest.file} exceeds budget ${largest.bytes} > ${MAX_BYTES}`
    );
    fail = true;
  }
  if (total > MAX_TOTAL_BYTES) {
    console.error(
      `Total bundle size exceeds budget ${total} > ${MAX_TOTAL_BYTES}`
    );
    fail = true;
  }
