...
const workers = [
  {
    name: 'onboarding',
    baseUrl: process.env.ONBOARDING_BASE_URL || 'https://atlasit-onboarding-prod.workers.dev',
    healthPath: '/health',
    authPath: '/api/onboarding/questions?industry=tech', // Public endpoint
  },
  {
    name: 'ai-orchestrator', 
    baseUrl: process.env.ORCHESTRATOR_BASE_URL || 'https://atlasit-orchestrator-prod.workers.dev',
    healthPath: '/health',
    authPath: '/status', // Requires API key
  },
  {
    name: 'documentation',
    baseUrl: process.env.DOCS_BASE_URL || 'https://atlasit-docs-prod.workers.dev', 
    healthPath: '/health',
...
async function smokeTest() {
  ...
  for (const worker of workers) {
    ...
    totalTests++;
    const healthResult = await testEndpoint(`${worker.baseUrl}${worker.healthPath}`);
    
    if (healthResult.ok && healthResult.hasRequestId) {
      console.log(`     ✅ Health OK (${healthResult.responseTime}ms, request-id present)`);
      passedTests++;
    } else {
      console.log(`     ❌ Health FAILED: ${healthResult.error || `Status ${healthResult.status}`}`);
    }
    ...
    if (authResult.ok) {
      ...
      console.log(`     ❌ Functional FAILED: ${authResult.error || `Status ${authResult.status}`}`);
    }
...
if (!process.env.ONBOARDING_BASE_URL && !process.env.ORCHESTRATOR_BASE_URL && !process.env.DOCS_BASE_URL) {
  console.log(`
