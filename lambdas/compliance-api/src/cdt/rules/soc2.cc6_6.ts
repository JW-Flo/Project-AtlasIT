...
export function evalSOC2_CC6_6(ev: CdtEvent): {
  ...
  return ok
    ? {
        decision: "pass",
        rationale: [`External systems enforce TLS ${tlsVersion}`],
        references: ["SOC2-CC6.6"],
      }
    : {
        decision: "fail",
        rationale: ["External systems do not enforce TLS 1.2+"],
        references: ["SOC2-CC6.6"],
      };
}
