...
export function evalISO_A9_2_6(ev: CdtEvent): {
  ...
  return offboardingCompletedHours <= 4
    ? {
        decision: "pass",
        rationale: [
          `All access rights removed within ${offboardingCompletedHours}h of termination`,
        ],
        references: ["ISO27001:A.9.2.6"],
      }
    : {
        decision: "fail",
        rationale: [`Access removal took ${offboardingCompletedHours}h (threshold: 4h)`],
        references: ["ISO27001:A.9.2.6"],
      };
}
