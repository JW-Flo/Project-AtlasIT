...
export function evalNIST_RS_CO_2(ev: CdtEvent): {
  ...
  return incidentReportingTimely
    ? {
        decision: "pass",
        rationale: ["Incidents reported within established SLA criteria"],
        references: ["NIST-CSF:RS.CO-2"],
      }
    : {
        decision: "fail",
        rationale: ["Incident reporting SLA not met"],
        references: ["NIST-CSF:RS.CO-2"],
      };
}
