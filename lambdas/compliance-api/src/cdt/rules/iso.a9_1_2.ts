...
export function evalISO_A9_1_2(ev: CdtEvent): {
  ...
  return networkAccessReviewed
    ? {
        decision: "pass",
        rationale: ["Network access rules reviewed and least-privilege applied"],
        references: ["ISO27001:A.9.1.2"],
      }
    : {
        decision: "fail",
        rationale: ["Network access rules not reviewed"],
        references: ["ISO27001:A.9.1.2"],
      };
}
