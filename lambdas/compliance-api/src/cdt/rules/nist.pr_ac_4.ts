...
export function evalNIST_PR_AC_4(ev: CdtEvent): {
  ...
  return leastPrivilegeReviewed
    ? {
        decision: "pass",
        rationale: ["Least-privilege access permissions reviewed and current"],
        references: ["NIST-CSF:PR.AC-4"],
      }
    : {
        decision: "fail",
        rationale: ["Least-privilege review not completed"],
        references: ["NIST-CSF:PR.AC-4"],
      };
}
