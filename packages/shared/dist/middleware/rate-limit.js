...
function resolveKvNamespace(c, bindingName) {
...
export function rateLimitMiddleware(config) {
    ...
    return async (c, next) => {
        ...
        c.header("X-RateLimit-Remaining", String(Math.max(0, limit - safeCurrent - 1)));
        c.header("X-RateLimit-Reset", String((windowBucket + 1) * parsed.windowSeconds));
        await next();
    };
}
