 ...
 */
export const ADAPTER_EVIDENCE_REGISTRY = [
    {
        slug: "github",
        evidenceTypes: [
            {
 ...
 */
export async function collectAdapterEvidence(adapterUrl, slug, tenantId) {
 ...
 */
export async function collectAllAdapterEvidence(adapterUrls, tenantId) {
    const configs = ADAPTER_EVIDENCE_REGISTRY.filter((c) => adapterUrls[c.slug]);
    const results = await Promise.allSettled(configs.map((c) => collectAdapterEvidence(adapterUrls[c.slug], c.slug, tenantId)));
    return results
...
export function parseControlRef(ref) {
...
//# sourceMappingURL=adapter-collector.js.map