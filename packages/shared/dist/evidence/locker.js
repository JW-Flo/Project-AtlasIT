 ...
 */
export async function storeEvidence(deps, evidence) {
    ...
    if (!alreadyExists) {
        ...
        for (const ctrl of evidence.controls) {
            ...
            batch.push(deps.db
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                .bind(rowId, evidence.tenantId, ctrl.framework, ctrl.framework, ctrl.controlId, ctrl.controlName, ctrl.category, evidence.source, contentHash, // link back to R2 via content hash
            evidence.actor, evidence.subject, JSON.stringify({
                impact: ctrl.impact,
 ...
 */
export async function queryEvidence(db, tenantId, options = {}) {
    ...
    const [rows, countRow] = await Promise.all([
        db
            .prepare(`SELECT * FROM compliance_evidence WHERE ${where}
