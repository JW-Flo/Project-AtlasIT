...
export function toErrorResponse(error) {
    ...
    if (error instanceof Error) {
        return new Response(JSON.stringify({ error: "INTERNAL", message: error.message }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    return new Response(JSON.stringify({ error: "UNKNOWN", message: "Unknown error" }), { status: 500, headers: { "Content-Type": "application/json" } });
}
