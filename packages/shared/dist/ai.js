...
export async function generateAI(messages, env, opts = {}) {
  const logger = new Logger({ service: "ai" });
  // Deterministic short‑circuit (env or opts flag)
  const deterministic = opts.deterministic || env.AI_DETERMINISTIC === "1";
  if (deterministic) {
  ...
  const primary = opts.provider || env.AI_PROVIDER || "cloudflare";
  // Resolve fallback chain: opts fallbackProviders overrides env.AI_FALLBACKS (comma list)
  const fallbackList = opts.fallbackProviders?.length
    ? [primary, ...opts.fallbackProviders]
    : [
        primary,
        ...(env.AI_FALLBACKS
  ...
  for (const providerName of fallbackList) {
    ...
    try {
      ...
      const result = await prov.generate(messages, opts);
