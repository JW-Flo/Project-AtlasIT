...
import { GroqAIProvider, type AIMessage, type AIOptions } from "./ai";
...
export async function runPromptGuard(
  ...
  try {
    const response = await provider.generate(
      ...
      { model: config.modelOverrides?.guard || ATLASIT_MODELS.GUARD, maxTokens: 100, temperature: 0 },
    );
...
const TASK_PATTERNS: {
...
}[] = [
  {
    ...
    keywords: ["audio", "transcribe", "speech", "voice", "recording", "whisper"],
  },
  {
    role: "infra",
    patterns: [
    ...
    keywords: [
      "terraform", "kubernetes", "k8s", "deploy", "cicd", "pipeline",
      "infrastructure", "automation", "config", "policy", "provision",
      "cloudflare", "wrangler", "worker", "compliance", "helm", "docker",
    ],
  },
  {
    role: "deep_review",
    ...
    keywords: [
      "review", "rca", "root cause", "analysis", "architecture", "design",
      "migration", "audit", "threat model", "post-mortem", "postmortem",
    ],
  },
  {
    role: "fast_ops",
    ...
    keywords: [
      "alert", "status", "health", "monitor", "summary", "check",
      "list", "show", "ops", "incident", "notification", "log",
    ],
  },
];
...
export interface AgentMeshResult {
  ...
  guardResult?: { safe: boolean; reason?: string };
  model: string;
  durationMs: number;
}
 ...
 */
export async function executeAgentMesh(
  input: string,
  config: AgentMeshConfig,
  systemPrompt?: string,
  ...
  let guardResult: { safe: boolean; reason?: string } | undefined;
...
function getAgentSystemPrompt(role: AgentRole): string {
  switch (role) {
    case "fast_ops":
