...
function wireErrorHandler(app: Hono) {
  app.onError((err, c) => {
    const correlationId =
      (() => {
        try {
...
describe("errorHandler middleware", () => {
  ...
  it("should include correlationId in response", async () => {
    ...
    wireErrorHandler(app);
    app.use("*", async (c, next) => {
      c.set("correlationId" as any, "test-corr-123");
      await next();
    });
    app.get("/test", () => {
  ...
  it("should log error details to console.error", async () => {
    ...
    const loggedJson = JSON.parse(spy.mock.calls[0][0] as string);
