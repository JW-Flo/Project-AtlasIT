...
export async function buildCopilotContext(
  db: { prepare(sql: string): { bind(...args: unknown[]): { all<T>(): Promise<{ results: T[] }>; first<T>(): Promise<T | null> } } },
  tenantId: string,
  ...
  const [
    ...
    // Selected frameworks
    db.prepare("SELECT value FROM tenant_preferences WHERE tenant_id = ? AND key = 'selected_frameworks'")
      .bind(tenantId).first<{ value: string }>(),

    // Compliance scores (latest per framework)
    db.prepare(
      `SELECT framework, overall_score FROM compliance_scores
       WHERE tenant_id = ? ORDER BY calculated_at DESC LIMIT 10`,
    ).bind(tenantId).all<{ framework: string; overall_score: number }>(),

    // Connected apps
    db.prepare(
      "SELECT app_id FROM connected_apps WHERE tenant_id = ? AND status = 'active'",
    ).bind(tenantId).all<{ app_id: string }>(),

    // Adapter health
    db.prepare(
      `SELECT adapter_slug, collected_at, items_count, error
       FROM adapter_collection_health WHERE tenant_id = ?`,
    ).bind(tenantId).all<{ adapter_slug: string; collected_at: string; items_count: number; error: string | null }>(),
    ...
    ).bind(tenantId).all<{ insight_type: string; severity: string; category: string | null; data: string; created_at: string }>(),
    ...
    ).bind(tenantId).first<{ total: number; open_count: number; overdue: number }>(),
    ...
    ).bind(tenantId).first<{ total: number; stale: number; recent: number }>(),

    // Automation rule count
    db.prepare(
      "SELECT COUNT(*) as cnt FROM automation_rules WHERE tenant_id = ? AND enabled = 1",
    ).bind(tenantId).first<{ cnt: number }>(),

    // Policy count
    db.prepare(
      "SELECT COUNT(*) as cnt FROM policies WHERE tenant_id = ?",
    ).bind(tenantId).first<{ cnt: number }>(),
  ...
  for (const row of scoresResult.results ?? []) {
    if (!seenFrameworks.has(row.framework)) {
      scores[row.framework] = row.overall_score;
...
export function formatContextForPrompt(ctx: CopilotTenantContext): string {
  const lines: string[] = [
    ...
    ctx.connectedApps.length > 0
      ? ctx.connectedApps.slice(0, 15).join(", ")
      : "None connected yet",
    ``,
    `### Evidence Pipeline`,
