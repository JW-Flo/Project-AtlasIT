...
// ---------------------------------------------------------------------------
// Portable storage interface (matches DO storage subset)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// State wrapping for DO constructor compatibility
// ---------------------------------------------------------------------------
...
export class JMLEngine {
   ...
   */
  async handleEnqueue(context: Record<string, unknown>): Promise<Response> {
    const type = context.type as WorkflowType | undefined;
    if (!type || !["joiner", "mover", "leaver"].includes(type)) {
      return this.jsonResponse(
        { error: "invalid_workflow_type", provided: type },
        400,
      );
    }
    ...
    const userId =
      (user?.id as string) ?? (context.subjectRef as string) ?? "user-unknown";

    const runId = crypto.randomUUID();
    const now = new Date().toISOString();

    const stepDefs = getStepDefinitions(type);
    const steps: StepState[] = stepDefs.map((def) => ({
      stepId: def.id,
      action: def.action,
      status: "pending",
      attempts: 0,
    ...
    const finalState = await this.storage.get<RunState>(`run:${runId}`);
  ...
  private async executeRun(runId: string): Promise<void> {
    const run = await this.storage.get<RunState>(`run:${runId}`);
    if (!run) return;
