        ...
        security: {
          ...
          authorization: {
            ...
            roles: [
              ...
              { id: "ops", name: "Operations", permissions: ["read:resources", "write:resources"] },
      ...
      "healthcare": {
        ...
        security: {
          ...
          authorization: {
            rbac: true,
            roles: [
              { id: "doctor", name: "Doctor", permissions: ["read:patient", "write:patient"] },
      ...
      "finance": {
        ...
        security: {
          ...
          authorization: {
            rbac: true,
            roles: [
              { id: "analyst", name: "Financial Analyst", permissions: ["read:reports"] },
              { id: "manager", name: "Finance Manager", permissions: ["read:reports", "write:reports"] },
              { id: "admin", name: "Administrator", permissions: ["*"] }
            ]
          },
          encryption: { atRest: true, inTransit: true }
      ...
      "retail": {
        ...
        security: {
          ...
          authorization: {
            rbac: true,
            roles: [
              { id: "customer", name: "Customer", permissions: ["read:products", "write:orders"] },
              { id: "staff", name: "Staff", permissions: ["read:orders", "write:inventory"] },
              { id: "admin", name: "Administrator", permissions: ["*"] }
            ]
          },
          encryption: { atRest: true, inTransit: true }
...
async function handleOnboarding(request, env) {
  try {
    ...
    if (existingState) {
      ...
      return json({ status: "success", tenantId, config: parsed.config, template: parsed.template, idempotent: true });
    }
    ...
    try {
      await env.DB.prepare("INSERT INTO audit_events (id, tenant_id, type, payload, created_at) VALUES (?, ?, ?, ?, ?)").bind(crypto.randomUUID(), tenantId, "onboarding.completed", JSON.stringify({ tenantId, industry }), (/* @__PURE__ */ new Date()).toISOString()).run();
    } catch (e) {
...
async function generateOnboardingQuestions(industry, requirements = []) {
  const baseQuestions = [
    { id: "company_size", question: "How many employees do you currently have?", type: "number" },
    { id: "mfa_policy", question: "Do you currently enforce MFA for all users?", type: "boolean" },
    { id: "primary_saas", question: "Which primary SaaS platforms do you rely on? (e.g., Google Workspace, Microsoft 365, Slack)", type: "multi-select" }
  ];
  if (industry.toLowerCase() === "healthcare") {
    baseQuestions.push({ id: "phi_storage", question: "Do you store or process Protected Health Information (PHI)?", type: "boolean" });
  }
  if (requirements.some((r) => r.toLowerCase().includes("compliance"))) {
    baseQuestions.push({ id: "audit_frequency", question: "How often do you perform internal access audits?", type: "select", options: ["Monthly", "Quarterly", "Annually", "Never"] });
  }
...
var Logger = class {
  ...
  debug(msg, meta) {
    if (shouldLog(this.level, "debug")) console.log(this.format("debug", msg, meta));
  }
  info(msg, meta) {
    if (shouldLog(this.level, "info")) console.log(this.format("info", msg, meta));
  }
  warn(msg, meta) {
    if (shouldLog(this.level, "warn")) console.warn(this.format("warn", msg, meta));
  }
  error(msg, meta) {
    if (shouldLog(this.level, "error")) console.error(this.format("error", msg, meta));
  }
};
    ...
    if (!envValidated) {
      try {
        ...
        if (logger && typeof logger.warn === "function")
          logger.warn("Env validation warning (non-fatal)", { error: String(e) });
      } finally {
        envValidated = true;
      }
    }
...
function handleHealth(cors) {
  ...
  return json2({ status: "healthy", service: "onboarding", timestamp: (/* @__PURE__ */ new Date()).toISOString(), version: "1.0.0" }, cors);
}
...
async function handleStart(request, cors) {
  ...
  return json2({ questions, version: "1.0.0" }, cors);
}
...
async function handleQuestions(url, cors) {
  ...
  return json2({ industry, count: questions.length, questions }, cors);
}
...
async function handleStatus(url, env, cors) {
  ...
  if (!tenantId)
    return json2(OnboardingErrors.TENANT_ID_REQUIRED(), cors, 400);
  const state = await env.STATE.get(`onboarding:${tenantId}`);
  if (!state)
    return json2(OnboardingErrors.ONBOARDING_NOT_FOUND(), cors, 404);
  return new Response(state, { headers: { "Content-Type": "application/json", ...cors } });
}
function notFound(url, method, corsHeaders) {
  return json2({ error: "Not Found", path: url.pathname, method }, corsHeaders, 404);
}
