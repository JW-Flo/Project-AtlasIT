import { TenantConfig, Template, TemplateFile } from '../types';

export async function generateTemplate(config: TenantConfig): Promise<Template> {
...
function generateMainConfig(config: TenantConfig): TemplateFile {
  const configContent = {
    version: '1.0.0',
...
function generateSecurityConfig(security: any): TemplateFile {
...
function generateIntegrationsConfig(integrations: any[]): TemplateFile {
...
function generateWorkflowsConfig(workflows: any[]): TemplateFile {
...
function generateDockerfile(config: TenantConfig): TemplateFile {
...
function generateReadme(config: TenantConfig): TemplateFile {
...
${config.workflows.map(workflow => 
  `- **${workflow.name}**: Triggered by ${workflow.trigger.type}, ${workflow.actions.length} actions`
).join('\n')}

## Security Roles

${config.security.authorization.roles.map(role => 
...
# Integrations
${config.integrations.map(integration => 
  `${integration.name.toUpperCase().replace(/\s+/g, '_')}_API_KEY=your-api-key`
).join('\n')}
\`\`\`
