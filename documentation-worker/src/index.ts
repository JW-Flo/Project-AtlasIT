...
function parseAndValidateDocument(bodyText: string): { valid: true; document: Record<string, unknown> } | { valid: false; error: string } {
...
export default {
	...
	async fetch(request: Request, env: WorkerEnv, ctx: ExecutionContext): Promise<Response> {
		...
		if (request.method === 'PUT' && url.pathname === '/docs') {
			if (!docsStore) {
				return json({ error: 'Docs storage is not configured', requestId }, { status: 500, headers: baseHeaders });
			}

			const contentLength = request.headers.get('content-length');
			if (contentLength && Number(contentLength) > MAX_DOCS_BYTES) {
				return json({ error: 'Payload too large', requestId }, { status: 400, headers: baseHeaders });
			}
