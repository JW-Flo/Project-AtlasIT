...
import { ProductionManagerAgent } from './production_manager_agent.js';
...
class AgentOrchestrator {
    ...
    initializeAgents() {
        ...
        this.agents.set('production', new ProductionManagerAgent());
    ...
    async startAllAgents() {
        ...
        for (const [name, agent] of this.agents) {
            try {
                ...
                await this.logging.logInfo('orchestrator', `${name} agent started successfully`);
            } catch (error) {
    ...
    async attemptAgentRestart(agentName, retries = 3) {
        ...
        await this.slack.sendAlert(`🔄 Attempting to restart ${agentName} agent...`);
        
        for (let i = 0; i < retries; i++) {
            try {
                await this.logging.logInfo('orchestrator', `Restart attempt ${i + 1}/${retries} for ${agentName}`);
                await agent.start();
                this.agentStatus.set(agentName, 'running');
                await this.slack.sendAlert(`✅ ${agentName} agent restarted successfully`);
                return;
            } catch (error) {
                await this.logging.logError('orchestrator', {
        ...
        await this.slack.sendAlert(`❌ ${agentName} agent failed to start after ${retries} attempts`);
    }
    ...
    monitorAgentStatus() {
        setInterval(async () => {
            ...
            if (hasIssues) {
                ...
                await this.slack.sendAlert(`⚠️ Agent Status Issues Detected:\n${summary}`);
            }
    ...
    async stopAllAgents() {
        ...
        for (const [name, agent] of this.agents) {
            try {
                ...
                await this.logging.logInfo('orchestrator', `${name} agent stopped successfully`);
            } catch (error) {
