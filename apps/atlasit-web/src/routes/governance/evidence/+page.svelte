<script lang="ts">
  ...
  async function search() {
    ...
    results = [];
    try {
      const r = await ComplianceAPI.searchEvidence({
        tenantId,
        pack,
        subject,
        limit,
      ...
      error = e?.body?.error || "Search failed";
    } finally {
      loading = false;
    }
  }
  ...
  async function verify() {
    ...
    error = null;
    try {
      verifyResult = await ComplianceAPI.verifyEvidence(verifyHash);
    } catch (e: any) {
      error = e?.body?.error || "Verify failed";
    }
  }
</script>
...
{:else}
  <table class="results">
    <thead
      ><tr><th>Hash</th><th>Pack</th><th>Subject</th><th>Created</th></tr
      ></thead
    >
    <tbody>
      {#each results as ev}
