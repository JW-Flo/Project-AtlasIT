<script lang="ts">
  ...
  async function load(reset = false) {
    ...
    error = null;
    try {
      const res = await listNotificationsFull({
        limit: 40,
        cursor: reset ? undefined : nextCursor || undefined,
      });
      const newItems = res.items || [];
      if (reset) items = newItems;
  ...
  >
    <div class="panel">
      <div class="panel-head">
        <h3>
          Notifications {#if unreadCount > 0}<span class="badge"
              >{unreadCount}</span
            >{/if}
        </h3>
        <div class="actions">
          <button disabled={marking || unreadCount === 0} on:click={markAll}
            ...
            {#each items as n}
              <li class="n-item {n.read ? 'read' : 'unread'}">
                <div class="top">
                  ...
                  >
                  <span class="time" title={n.createdAt}
                    >{relativeTime(n.createdAt)}</span
                  >
                </div>
                ...
                <div class="row-actions">
                  {#if !n.read}
                    <button
                      disabled={marking}
                      on:click={() => markSelected([n.id])}>Mark Read</button
                    >
                  {/if}
                </div>
              </li>
