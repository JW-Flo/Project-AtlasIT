<script lang="ts">
  ...
  async function load(reset = false) {
    ...
    error = null;
    try {
      const res: EvidenceSearchResponse = await searchEvidence({
...
{#if open}
  ...
  >
    <div class="drawer">
      <!-- retain existing drawer styling inside dialog for now -->
      <div class="header">
            ...
            {#each items as ev}
              <li class="ev-item">
                <div class="meta">
                  ...
                  <span class="time" title={ev.createdAt}
                    >{relativeTime(ev.createdAt)}</span
                  >
                </div>
                <div class="actions">
                  <button
                    disabled={verifying === ev.hash}
                    on:click={() => onVerify(ev.hash)}
                  >
