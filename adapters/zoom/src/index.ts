...
import { handleZoomWebhook } from "./webhooks.js";
...
app.post("/api/sync", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind(syncId, body.tenantId, "zoom", "running", new Date().toISOString())
    .run();
  ...
  const tokenRow = await c.env.DB.prepare(
    `SELECT access_token FROM connector_tokens
     WHERE tenant_id = ?1 AND connector_slug = 'zoom'
  ...
  // Retrieve Zoom account config (optional)
  const configRow = await c.env.DB.prepare(
    `SELECT config FROM connector_configs
     WHERE tenant_id = ?1 AND connector_slug = 'zoom' LIMIT 1`,
  )
    .bind(body.tenantId)
    .first<{ config: string }>();

  if (configRow) {
    const config = JSON.parse(configRow.config) as { accountId?: string };
...
app.get("/api/status", async (c) => {
  ...
  const recentSyncs = await c.env.DB.prepare(
    `SELECT id, status, created_at, completed_at
     FROM sync_jobs WHERE tenant_id = ?1 AND connector_slug = 'zoom'
...
// Zoom webhook receiver (from Zoom API)
app.post("/webhooks/zoom/events", (c) => handleZoomWebhook(c));
