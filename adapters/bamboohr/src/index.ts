...
import { handleBambooHRWebhook } from "./webhooks.js";
...
app.post("/api/sync", async (c) => {
  ...
  const configRow = await c.env.DB.prepare(
    `SELECT config FROM connector_configs
     WHERE tenant_id = ?1 AND connector_slug = 'bamboohr' LIMIT 1`,
  )
    .bind(body.tenantId)
    .first<{ config: string }>();
  ...
  if (!configRow) {
    ...
    return c.json(
      {
        error: "No connector config found. Configure subdomain first.",
        correlationId,
      },
      400,
    );
...
app.get("/api/status", async (c) => {
  ...
  const recentSyncs = await c.env.DB.prepare(
    `SELECT id, status, created_at, completed_at
     FROM sync_jobs WHERE tenant_id = ?1 AND connector_slug = 'bamboohr'
...
app.post("/webhooks/bamboohr/events", (c) => handleBambooHRWebhook(c));
