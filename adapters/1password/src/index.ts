...
import { handleOnePasswordWebhook } from "./webhooks.js";
...
app.post("/api/sync", async (c) => {
  ...
  const token = c.env.ONEPASSWORD_SERVICE_TOKEN;
  if (!token) {
    await c.env.DB.prepare(
    ...
    return c.json(
      {
        error: "1Password service token not configured",
        correlationId,
      },
      400,
    );
  ...
  const configRow = await c.env.DB.prepare(
    `SELECT config FROM connector_configs
     WHERE tenant_id = ?1 AND connector_slug = '1password' LIMIT 1`,
  )
    .bind(body.tenantId)
    .first<{ config: string }>();
  ...
  const finalConfig = applyDefaults(
    config as unknown as Record<string, unknown>,
  );
  const scimBridgeUrl = finalConfig.scimBridgeUrl;

  if (!scimBridgeUrl || typeof scimBridgeUrl !== "string") {
    return c.json(
      { error: "scimBridgeUrl is required in config", correlationId },
      400,
    );
  }
...
app.get("/api/status", async (c) => {
  ...
  const recentSyncs = await c.env.DB.prepare(
    `SELECT id, status, created_at, completed_at
     FROM sync_jobs WHERE tenant_id = ?1 AND connector_slug = '1password'
...
// Webhook handler (not supported for 1Password)
app.post("/webhooks/1password/events", (c) => handleOnePasswordWebhook(c));
