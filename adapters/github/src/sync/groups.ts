import type { GitHubTeam, SyncResult } from "../types.js";
...
export async function syncGroups(
  ...
  const teams = await listTeams(org, accessToken);
  ...
  for (const team of teams) {
    ...
    await syncTeamMemberships(accessToken, org, team, db, tenantId);
  }
...
async function syncTeamMemberships(
  ...
  const members = await getTeamMembers(org, team.slug, accessToken);
