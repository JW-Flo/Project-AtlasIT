...
import { handleCrowdStrikeWebhook } from "./webhooks.js";
...
// Trigger a full directory sync (users + devices + host groups)
app.post("/api/sync", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind(syncId, body.tenantId, "crowdstrike", "running", new Date().toISOString())
    .run();
  ...
  const configRow = await c.env.DB.prepare(
    `SELECT config FROM connector_configs
     WHERE tenant_id = ?1 AND connector_slug = 'crowdstrike' LIMIT 1`,
  )
    .bind(body.tenantId)
    .first<{ config: string }>();
  ...
  if (!configRow) {
    ...
    return c.json(
      {
        error: "No connector config found. Configure clientId, clientSecret, and baseUrl first.",
        correlationId,
      },
      400,
    );
...
app.get("/api/status", async (c) => {
  ...
  const recentSyncs = await c.env.DB.prepare(
    `SELECT id, status, created_at, completed_at
     FROM sync_jobs WHERE tenant_id = ?1 AND connector_slug = 'crowdstrike'
...
// CrowdStrike detection/event webhook receiver
app.post("/webhooks/crowdstrike/events", (c) => handleCrowdStrikeWebhook(c));
