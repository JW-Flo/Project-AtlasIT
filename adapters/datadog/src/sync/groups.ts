import type { DatadogGroup, SyncResult } from "../types.js";
...
export async function syncGroups(
  ...
  const groups = await listGroups(apiKey, appKey);
  ...
  for (const group of groups) {
    ...
    await db
      ...
      .bind(
        ...
        group.attributes.description ?? null,
      )
      .run();
    ...
    await syncGroupMemberships(apiKey, appKey, group, db, tenantId);
  }
...
async function syncGroupMemberships(
  ...
  const members = await getGroupMembers(group.id, apiKey, appKey);
