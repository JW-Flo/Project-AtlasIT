...
import { handleXeroWebhook } from "./webhooks.js";
...
app.get("/health", (c) => {
  return c.json({
    ...
    connector: {
      ...
      name: "Xero",
      provider: "Xero",
      capabilities: ["contact-sync", "user-sync"],
    },
  });
});
...
app.post("/api/sync", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind(syncId, body.tenantId, "xero", "running", new Date().toISOString())
    .run();
  ...
  const tokenRow = await c.env.DB.prepare(
    `SELECT access_token FROM connector_tokens
     WHERE tenant_id = ?1 AND connector_slug = 'xero'
  ...
  const configRow = await c.env.DB.prepare(
    `SELECT config FROM connector_configs
     WHERE tenant_id = ?1 AND connector_slug = 'xero' LIMIT 1`,
  )
    .bind(body.tenantId)
    .first<{ config: string }>();
...
app.get("/api/status", async (c) => {
  ...
  const recentSyncs = await c.env.DB.prepare(
    `SELECT id, status, created_at, completed_at
     FROM sync_jobs WHERE tenant_id = ?1 AND connector_slug = 'xero'
...
app.post("/webhooks/xero/events", (c) => handleXeroWebhook(c));
