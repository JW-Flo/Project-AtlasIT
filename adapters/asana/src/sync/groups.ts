import type { AsanaTeam, SyncResult } from "../types.js";
...
export async function syncGroups(
  ...
  const teams = await listTeams(workspaceGid, accessToken);
  ...
  for (const team of teams) {
    ...
    await syncTeamMemberships(accessToken, team, db, tenantId);
  }
...
async function syncTeamMemberships(
  ...
  const members = await getTeamMembers(team.gid, accessToken);
