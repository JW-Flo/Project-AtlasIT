...
import { handleDropboxWebhook } from "./webhooks.js";
...
app.post("/api/sync", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind(syncId, body.tenantId, "dropbox", "running", new Date().toISOString())
    .run();
  ...
  const tokenRow = await c.env.DB.prepare(
    `SELECT access_token FROM connector_tokens
     WHERE tenant_id = ?1 AND connector_slug = 'dropbox'
  ...
  const configRow = await c.env.DB.prepare(
    `SELECT config FROM connector_configs
     WHERE tenant_id = ?1 AND connector_slug = 'dropbox' LIMIT 1`,
  )
    .bind(body.tenantId)
    .first<{ config: string }>();

  // Config is optional for Dropbox
...
app.get("/api/status", async (c) => {
  ...
  const recentSyncs = await c.env.DB.prepare(
    `SELECT id, status, created_at, completed_at
     FROM sync_jobs WHERE tenant_id = ?1 AND connector_slug = 'dropbox'
...
app.post("/webhooks/dropbox/events", (c) => handleDropboxWebhook(c));
