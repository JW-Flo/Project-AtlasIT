import type { DropboxGroup, SyncResult } from "../types.js";
import {
  listTeamGroups,
  continueListingTeamGroups,
  getGroupMembers,
} from "../client.js";
...
export async function syncGroups(
  ...
  while (hasMore) {
    const response = cursor
      ? await continueListingTeamGroups(accessToken, cursor)
      : await listTeamGroups(accessToken);
    ...
    for (const group of response.groups) {
      ...
      await syncGroupMemberships(accessToken, group.group_id, db, tenantId);
    }
