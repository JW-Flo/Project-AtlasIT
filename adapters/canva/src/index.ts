...
import { handleCanvaWebhook } from "./webhooks.js";
...
app.post("/api/sync", async (c) => {
  ...
  const configRow = await c.env.DB.prepare(
    `SELECT config FROM connector_configs
     WHERE connector_slug = 'canva' AND tenant_id = ? LIMIT 1`,
  )
    .bind(body.tenantId)
    .first<{ config: string }>();
  ...
  const credentialRow = await c.env.DB.prepare(
    `SELECT credential_data FROM credentials
     WHERE tenant_id = ? AND connector_slug = 'canva' LIMIT 1`,
  )
    .bind(body.tenantId)
  ...
  try {
    ...
    // Sync groups (Canva has limited group support)
...
app.get("/auth/authorize", async (c) => {
  ...
  const authUrl = getAuthorizationUrl(
    {
      CANVA_CLIENT_ID: c.env.CANVA_CLIENT_ID,
...
app.get("/auth/callback", async (c) => {
  ...
  try {
    const token = await exchangeCodeForToken(
      {
        CANVA_CLIENT_ID: c.env.CANVA_CLIENT_ID,
        CANVA_CLIENT_SECRET: c.env.CANVA_CLIENT_SECRET,
        OAUTH2_REDIRECT_URI: c.env.OAUTH2_REDIRECT_URI,
      },
      code,
    );
...
// Canva webhook receiver (from Canva API)
app.post("/webhooks/canva", handleCanvaWebhook);
