...
import { handleADPWebhook } from "./webhooks.js";
...
app.post("/api/sync", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind(syncId, body.tenantId, "adp", "running", new Date().toISOString())
    .run();
  ...
  const tokenRow = await c.env.DB.prepare(
    `SELECT access_token FROM connector_tokens
     WHERE tenant_id = ?1 AND connector_slug = 'adp'
  ...
  // Retrieve connector config (SSL certs are in env bindings)
  const configRow = await c.env.DB.prepare(
    `SELECT config FROM connector_configs
     WHERE tenant_id = ?1 AND connector_slug = 'adp' LIMIT 1`,
  )
    .bind(body.tenantId)
    .first<{ config: string }>();
  ...
  if (!configRow) {
    ...
    return c.json(
      {
        error: "No connector config found. Configure ADP connection first.",
        correlationId,
      },
      400,
    );
...
app.get("/api/status", async (c) => {
  ...
  const recentSyncs = await c.env.DB.prepare(
    `SELECT id, status, created_at, completed_at
     FROM sync_jobs WHERE tenant_id = ?1 AND connector_slug = 'adp'
...
// ADP event notification webhook receiver
app.post("/webhooks/adp/events", (c) => handleADPWebhook(c));
