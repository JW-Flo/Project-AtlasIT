...
app.post("/api/sync", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind(syncId, body.tenantId, "notion", "running", new Date().toISOString())
    .run();

  // Retrieve stored OAuth token
  const tokenRow = await c.env.DB.prepare(
    `SELECT access_token FROM connector_tokens
     WHERE tenant_id = ?1 AND connector_slug = 'notion'
  ...
  if (!tokenRow) {
    await c.env.DB.prepare(
      ...
      .run();
    return c.json(
      { error: "No OAuth token found. Authorize first.", correlationId },
      400,
    );
  }
