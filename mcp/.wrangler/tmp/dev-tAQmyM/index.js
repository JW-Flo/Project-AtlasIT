...
// .wrangler/tmp/bundle-naDGiw/checked-fetch.js
...
// .wrangler/tmp/bundle-naDGiw/strip-cf-connecting-ip-header.js
...
app.post("/register", async (c) => {
  ...
  const agents = await cached("agents", 5e3, async () => {
    const raw2 = await c.env.MCP_STORE.get("agents");
...
app.post("/task", async (c) => {
  ...
  const tasks = await cached("tasks", 5e3, async () => {
    const raw2 = await c.env.MCP_STORE.get("tasks");
    return raw2 ? JSON.parse(raw2) : [];
  });
  tasks.push(task);
...
app.post("/plan/propose", async (c) => {
  ...
  plan.status = "pending";
  plan.proposed_at = (/* @__PURE__ */ new Date()).toISOString();
...
app.post("/plan/approve", async (c) => {
  ...
  plan.approved_at = (/* @__PURE__ */ new Date()).toISOString();
  await c.env.MCP_STORE.put("latest_plan", JSON.stringify(plan));
  return c.json({ status: "success", plan });
});
app.get("/metrics", async (c) => {
  const [agents, tasks, plan] = await Promise.all([
    cached("agents", 5e3, () => c.env.MCP_STORE.get("agents").then((r) => r ? JSON.parse(r) : [])),
    cached("tasks", 5e3, () => c.env.MCP_STORE.get("tasks").then((r) => r ? JSON.parse(r) : [])),
    cached("plan", 5e3, () => c.env.MCP_STORE.get("latest_plan").then((r) => r ? JSON.parse(r) : null))
  ]);
  return c.json({
...
var scheduled = /* @__PURE__ */ __name(async (event, env, ctx) => {
  ctx.waitUntil(Promise.all([
    env.MCP_STORE.get("agents").then((r) => cache.set("agents", { val: r ? JSON.parse(r) : [], exp: Date.now() + 6e4 })),
    env.MCP_STORE.get("tasks").then((r) => cache.set("tasks", { val: r ? JSON.parse(r) : [], exp: Date.now() + 6e4 })),
    env.MCP_STORE.get("latest_plan").then((r) => cache.set("plan", { val: r ? JSON.parse(r) : null, exp: Date.now() + 6e4 }))
  ]));
}, "scheduled");
...
// .wrangler/tmp/bundle-naDGiw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-naDGiw/middleware-loader.entry.ts
