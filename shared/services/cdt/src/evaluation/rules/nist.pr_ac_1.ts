...
export function evalNIST_PR_AC_1(ev: CdtEvent): { decision: "pass"|"fail"|"unknown", rationale: string[], references: string[] } {
  ...
  if (idpConnected && automatedLifecycle) {
    return { decision: "pass", rationale: ["IdP connected with automated identity lifecycle management"], references: ["NIST-CSF:PR.AC-1"] };
  }
  const issues: string[] = [];
