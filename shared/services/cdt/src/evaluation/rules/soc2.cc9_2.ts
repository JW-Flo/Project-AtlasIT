...
export function evalSOC2_CC9_2(ev: CdtEvent): { decision: "pass"|"fail"|"unknown", rationale: string[], references: string[] } {
  ...
  if (bcrExists && daysSinceBcrTest <= 365) {
    return { decision: "pass", rationale: ["BCR/DR plan exists and tested within 365 days"], references: ["SOC2-CC9.2"] };
  }
  const issues: string[] = [];
