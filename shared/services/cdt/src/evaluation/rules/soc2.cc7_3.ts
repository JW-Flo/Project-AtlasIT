...
export function evalSOC2_CC7_3(ev: CdtEvent): { decision: "pass"|"fail"|"unknown", rationale: string[], references: string[] } {
  ...
  if (planExists && daysSinceTest <= 365) {
    return { decision: "pass", rationale: ["Incident response plan exists and tested within last 365 days"], references: ["SOC2-CC7.3"] };
  }
  const issues: string[] = [];
