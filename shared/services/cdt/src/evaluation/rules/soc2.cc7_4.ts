...
export function evalSOC2_CC7_4(ev: CdtEvent): { decision: "pass"|"fail"|"unknown", rationale: string[], references: string[] } {
  ...
  if (openCriticalIncidents === 0 && meanTimeToContainHours <= 4) {
    return { decision: "pass", rationale: ["No open critical incidents; MTTC within 4h SLA"], references: ["SOC2-CC7.4"] };
  }
  const issues: string[] = [];
