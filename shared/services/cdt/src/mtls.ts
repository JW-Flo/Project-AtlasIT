...
export async function validateClientCert(req: Request, allowedFingerprints: string[] | undefined) {
  if (!allowedFingerprints || allowedFingerprints.length === 0) return { ok: true, reason: 'no-allowlist-configured' } as const;
