...
export default {
  async fetch(req: Request, env: any): Promise<Response> {
    ...
    if (url.pathname === "/twin/event" && req.method === "POST") {
      if (env.ALLOWED_CLIENT_CERT_FPS) {
        const allow = String(env.ALLOWED_CLIENT_CERT_FPS).split(',').map((s: string)=>s.trim()).filter(Boolean);
        if (allow.length) {
          const certOk = await validateClientCert(req, allow);
          if (!certOk.ok) {
            telemetry.log("warn","cdt.auth.mtls_deny", { reason: certOk.reason, fingerprint: (certOk as any).fingerprint });
      ...
      if (token && !('key' in token)) {
        return new Response(JSON.stringify(token), { headers: { "content-type":"application/json", "x-idempotent-replay":"true" } });
      }
      const ev = await req.json() as CdtEvent;
      const controls = [
        "SOC2-CC6.2","ISO-27001-A.9.2.3","SOC2-CC1.1","ISO-27001-A.12.6.1","ISO-27001-A.9.4.2","SOC2-CC7.2","ISO-27001-A.13.1.1"
      ];
      ...
      for (const cid of controls) {
        ...
        const updated = await controlRepo.upsert(ev.tenant, cid, (cur) => {
          const base: ControlState = cur ?? { tenant: ev.tenant, control_id: cid, state: "unknown", updated_at: new Date().toISOString(), evidence_ids: [], version: 0 };
        ...
        if (before !== updated.state) {
          const evidence = await evidenceRepo.writeEvidence({ event: ev, result: res, before, after: updated.state }, { control_id: cid, tenant: ev.tenant, producer: "cdt.eval", trace_id: ev.trace_id });
          updated.evidence_ids.push(evidence.id);
          await controlRepo.put(updated);
        }
        telemetry.metric("cdt.eval.count", 1, { control_id: cid, decision: updated.state });
        telemetry.log("info", "cdt.eval", { trace_id: ev.trace_id, tenant: ev.tenant, control_id: cid, decision: updated.state, latency_ms: Date.now()-t0 });
  ...
  if (token && 'key' in token) await persistIdempotency(IDEMP_NS, token as any, payload);
    ...
    if (url.pathname.startsWith("/twin/remediate/") && req.method === "POST") {
      ...
      if (!tenant) return new Response(JSON.stringify({ error: "tenant required" }), { status: 400, headers: { "content-type":"application/json" } });
      const runner = makeRemediationRunner(queue);
      await runner.enqueue(tenant, cid, body.context ?? {});
      telemetry.log("info", "cdt.remediation.enqueue", { tenant, control_id: cid });
      return new Response(JSON.stringify({ enqueued: true }), { status: 202, headers: { "content-type":"application/json" } });
    }
    ...
    if (m && req.method === "GET") {
      ...
      return new Response(JSON.stringify(cs ?? null), { headers: { "content-type": "application/json" } });
    }
